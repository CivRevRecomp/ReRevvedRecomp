...
3. In the content selector, choose your game ISO or select an already-extracted
   content folder.
...
ReRevved validates required files before launch and reports incomplete,
modified, wrong-version, or wrong-region content. The accepted location is
written to rerevved.toml, so the selector normally appears only once.

USER FILES AND BACKUPS

Config, logs, saves, and caches are stored in the per-user data folder:

  Documents\rerevved        Windows
  ~/Documents/rerevved      Linux, or the XDG documents directory

To back up progress and settings, exit ReRevved and copy the per-user folder.
