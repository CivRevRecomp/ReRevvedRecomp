...
`rexglue-sdk` checkout. The driver never modifies that checkout.
...
per-user `rerevved.toml` and reused on later runs.
...
`resolution` preset sizes the window; Civilization Revolution still renders a
fixed 1280x720 guest image. Use `resolution_scale` for higher-quality rendering:
...
these settings in the per-user `rerevved.toml`.
...
`../rexglue-sdk/out/install/win-amd64-1.0`. Generated guest code lives under
`generated/default/`. Build output, development user data, caches, and logs are
ignored.

A successful build proves compilation only. A runtime claim requires a captured
milestone or an owner-observed visible result. `-ProbeSeconds <n>` bounds a smoke
test. `-Interactive` waits for normal game exit.
